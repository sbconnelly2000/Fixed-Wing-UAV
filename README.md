...
An autonomous delta-wing Unmanned Aerial Vehicle (UAV) flight controller developed in C++ for the ESP32 platform[cite: 1, 3]. Designed for multi-axis flight stability, the system integrates real-time Proportional-Derivative (PD) control loops[cite: 1, 3], sensor fusion via I2C and SPI interfaces, dynamic pressure airspeed estimation, elevon actuation mixing, non-blocking UBX protocol GNSS navigation, and high-frequency SD card telemetry logging[cite: 1, 2].

---

## Technical Features

* **Real-Time PD Control Architecture:** Multi-axis attitude stabilization loop operating on absolute Euler orientation vectors and angular velocity feedback[cite: 1, 3].
...
* **High-Rate Telemetry Engine:** Non-blocking u-blox GNSS integration over serial combined with 10 Hz telemetry logging of 17 flight parameters to onboard SD storage[cite: 1, 2].

---

## System Hardware Architecture

The system's physical component placement and power distribution were configured to maintain center-of-gravity (CG) stability, minimize parasitic wiring weight, and optimize flight endurance[cite: 3].
...
| **IMU Sensor** | Adafruit BNO055 (I2C `0x28`) | 9-DOF orientation tracking (Euler angles and angular body rates)[cite: 1] |
| **Barometric Sensor** | Adafruit BMP280 (SPI) | Ambient air temperature and pressure sampling for air density calibration[cite: 1] |
| **Airspeed Indicator** | Analog Pitot Differential Pressure Sensor (`A0`) | Measure dynamic pressure differential for airspeed calculation[cite: 1] |
| **GNSS Module** | u-blox GNSS (UART1 @ 38400 Baud) | Latitude, longitude, altitude, and ground speed tracking using UBX frame format[cite: 1] |
| **Data Logging** | SPI MicroSD Storage (`GPIO 21`) | High-frequency telemetry logging (`/Flight_Data.csv`)[cite: 1] |
| **Flight Actuators** | Servo Motors (`GPIO D1`, `GPIO D2`) | Elevon control surface position control[cite: 1] |
...
The flight controller computes error between target reference states and feedback states derived from the BNO055 IMU[cite: 1]. Control outputs for roll and pitch stability combine proportional angle error and derivative body rate feedback[cite: 1]:
...
The aircraft underwent systematically recorded flight testing to evaluate open-loop flight dynamics and closed-loop autonomous stability[cite: 2, 3]. Flight telemetry was logged continuously at 10 Hz[cite: 1, 2].
...
* **Roll Dynamics & Overshoot:** Initial autonomous flight tests revealed severe roll oscillations and dramatic rolling movements caused by an over-aggressive roll controller gain setting[cite: 2]. Downscaling the gain mitigated limit cycling[cite: 2].
* **Pitch Dynamics & Nose-Dive Recovery:** Flight 1 resulted in an immediate nose-dive upon launch due to an untuned baseline pitch gain[cite: 2]. Re-tuning the pitch loop gain resolved the pitch instability and allowed level flight hold in subsequent flights[cite: 2].
